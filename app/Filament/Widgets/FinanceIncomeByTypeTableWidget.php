...
class FinanceIncomeByTypeTableWidget extends ArrayRecordsTableWidget
{
    protected static ?string $heading = 'Поступления по типам';
    ...
    {
        return collect(app(FinanceAnalyticsService::class)->build($this->resolvePeriod())['income_by_type'] ?? [])
            ->values()
            ->all();
    }
    ...
    public function table(Table $table): Table
    {
        return parent::table($table)
            ...
            ->emptyStateDescription('Поступления появятся после импорта или ручного добавления транзакций.');
    }
    ...
    protected function getTableColumns(): array
    {
        return [
            ...
            TextColumn::make('share')->label('Доля от выручки')->formatStateUsing(fn ($state) => $state === null ? 'Нет данных' : number_format((float) $state, 1, ',', ' ').'%')->sortable(),
